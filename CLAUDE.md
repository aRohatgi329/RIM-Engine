...
- **New features go in new files only** — do not modify `data/fmp.py`, `models/rim.py`, `tabs/earnings_tab.py`, or other existing files when adding features unless explicitly instructed. **Active exception (branch `learning-page-updates` only): `tabs/learning_tab.py` may be modified directly. No other file. Remove this exception on merge.**
